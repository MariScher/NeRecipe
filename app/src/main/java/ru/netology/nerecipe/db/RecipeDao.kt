...
interface RecipeDao {
    ...
    @Query("UPDATE recipes SET title = :title, textRecipe = :textRecipe, categoryRecipe = :categoryRecipe WHERE id = :id")
    fun updateById(
    ...
    fun save(recipe: RecipeEntity) =
        if (recipe.id == RecipeRepository.NEW_ID)
            insert(recipe)
        else updateById(recipe.id, recipe.title, recipe.textRecipe, recipe.categoryRecipe)
